...
db.serialize(() =>{
    db.run('CREATE TABLLE IF NOT EXISTS products (product_id INTEGER PRIMARY KEY AUTOINCREMENT, product_name TEXT, product_img TEXT, product_price INTEGER, product_description TEXT )');
    db.run('INSERT INTO products (product_name, product_img, product_price, product_description) VALUES (?,?,?,o?), ("Poke Bowl Saumon", "poke-bowl-saumon-detox.png", 12, "Saumon, Haricot")');
    ...
    db.run('CREATE TABLLE IF NOT EXISTS category (category_id INTEGER PRIMARY KEY AUTOINCREMENT, cartegory_name TEXT, product_id INTEGER, FOREIGN KEY(product_id) REFERENCES product(id) )')
